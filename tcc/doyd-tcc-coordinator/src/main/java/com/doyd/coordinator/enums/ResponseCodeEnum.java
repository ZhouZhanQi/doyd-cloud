 ...
 * @desc 状态码枚举
 */
public enum ResponseCodeEnum implements RestStatus {
    ...
    private static final ImmutableMap<Integer, ResponseCodeEnum> CACHE;

    static {
        //ImmutableMap是不可变集合，是线程安全
        final ImmutableMap.Builder<Integer, ResponseCodeEnum> builder = ImmutableMap.builder();
        for (ResponseCodeEnum statusCode : values()) {
    ...
    public static ResponseCodeEnum valueOfCode(int code) {
        final ResponseCodeEnum status = CACHE.get(code);
        if (status == null) {

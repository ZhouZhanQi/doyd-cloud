...
import com.doyd.eventdriven.config.MessageRoute;
import com.doyd.eventdriven.entity.Event;
...
import com.doyd.eventdriven.strategy.BatchFetchEventStrategy;
import com.doyd.eventdriven.strategy.PublishNewEventStrategy;
import com.doyd.eventdriven.strategy.RepublishPendingEventStrategy;
...
public class EventDrivenPublisher {
    ...
    public int persistPublishMessage(Object payload, String businessType) {
        ...
        publisher.setEventStatus(EventStatusEnum.NEW);
        publisher.setGuid(UUID.randomUUID().toString());
        publisher.setLockVersion(0);
        publisher.setPayload(JacksonUtils.pojoTojson(payload));
...
//        HibernateValidators.throwsIfInvalid(publisher);
    ...
    private class RabbitReturnCallback implements RabbitTemplate.ReturnCallback {
        ...
        public void returnedMessage(Message message, int replyCode, String replyText, String exchange, String routingKey) {
            ...
            try {
                ...
                final String guid = JacksonUtils.readField(failedMessage,  "guid");
                final EventPublisher publisher = new EventPublisher();
                publisher.setGuid(guid);
                if (EventStatusEnum.NO_ROUTE.name().equalsIgnoreCase(replyText)) {
                    publisher.setEventStatus(EventStatusEnum.NO_ROUTE);
                } else {
                    logReturnedFault(replyCode, replyText, exchange, routingKey, failedMessage);
                    publisher.setEventStatus(EventStatusEnum.ERROR);
                }
    ...
    private class RabbitConfirmCallback implements RabbitTemplate.ConfirmCallback {
        ...
        public void confirm(CorrelationData correlationData, boolean ack, String cause) {
            ...
            if (ack) {
                // flag instead
                publisherMapper.updateEventStatusByPrimaryKeyInCasMode(id, EventStatusEnum.PENDING, EventStatusEnum.DONE);
                // 或直接删除
                ...
                publisherMapper.updateEventStatusByPrimaryKeyInCasMode(id, EventStatusEnum.PENDING, EventStatusEnum.NOT_FOUND);
            }
        }
    }

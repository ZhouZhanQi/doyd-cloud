...
@Mapper
public interface EventPublisherMapper {

    int insertSelective(EventPublisher publisher);
    /**
     * 获取定量的指定状态的事件群，用于发往Broker
     */
    Set<EventPublisher> selectLimitedEntityByEventStatus(@Param("eventStatus") EventStatusEnum eventStatus, @Param("limited") int limited);

    /**
     * 获取定量的、指定状态的、在指定更新时间前的事件群，用于Broker没有confirm某消息的情况下进行重新发送，目的是防止在单节点的情况因为意外宕机而导致PENDING事件没有转换为其他状态的能力。
     */
    Set<EventPublisher> selectLimitedEntityByEventStatusBeforeTheSpecifiedUpdateTime(@Param("eventStatus") EventStatusEnum eventStatus, @Param("limited") int limited, @Param("beforeThisTime") OffsetDateTime beforeThisTime);

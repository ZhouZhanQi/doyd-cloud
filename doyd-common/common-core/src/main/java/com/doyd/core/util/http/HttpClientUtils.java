...
public class HttpClientUtils {
    ...
    public static String doJsonPost(String url, String body) {
        ...
        headers.put("Content-Type", "application/com.zhouzq.json;charset=utf-8");
     ...
     * @return
     */
    public static byte[] resp2bytes(CloseableHttpResponse response) {
        // 将响应内容转换为字符串
        int status = response.getStatusLine().getStatusCode();

        try {
            byte[] respBuffer = EntityUtils.toByteArray(response.getEntity());
            if (!is2xxSuccessfulStatus(status)) {
                throw new RuntimeException("请求响应码 " + status + ": " + respBuffer);
            }
            return respBuffer;
        } catch (IOException e) {
            throw new RuntimeException("读取文件响应流出错");
        }
    }
